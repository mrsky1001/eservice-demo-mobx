...
export const handlerSuccess = (res: AxiosResponse, handler?: (val: any) => any, isSuccessMessage = true, isShowMessage = true): void => {
    if (res.data.success) {
        handler && handler(res.data.data)

        if (res.config.method !== 'get' && isSuccessMessage && isShowMessage) {
